...
public class 	TestProductControllerIT {
	...
	public void ProductDetailAddItemIntegrationTest() throws Exception {
		...
		//testing the cart button total
		String cartheaderbuttontotal = cartPage.getHtmlElementById("cartheaderbuttontotal").getTextContent();
		...
		//testing the button total and format.
		assertThat(cartheaderbuttontotal).isEqualTo("$" + new DecimalFormat("#0.##").format(PRODUCT_PRICE.multiply(new BigDecimal(ORDER_QUANTITY))));


		//testing update page
		HtmlForm updateForm = cartPage.getFirstByXPath("//form[@action='/cart/update']");
		HtmlInput updateQuantityInput = updateForm.getInputByName("newQuantity");
		updateQuantityInput.setValueAttribute("1");
		HtmlAnchor updateAnchor = cartPage.getAnchorByText("Update");
		HtmlPage updatePage = updateAnchor.click();

		String updateFlashMessage = updatePage.getHtmlElementById("flashmessage").getTextContent();
		assertThat(updateFlashMessage).isEqualTo("Updated " + PRODUCT_NAME + " to " + "1");

		//remove item
		HtmlAnchor removeAnchor = updatePage.getAnchorByText("Remove Item");
		HtmlPage removeItem = removeAnchor.click();
		String removeFlashMessage = removeItem.getHtmlElementById("flashmessage").getTextContent();
		assertThat(removeFlashMessage).isEqualTo("Removed " +PRODUCT_NAME);
