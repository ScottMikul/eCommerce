...
public class CheckoutController{
	...
	String postCouponCode(Model model,@Valid @ModelAttribute(value="couponCode") CouponCode couponCode,
						  BindingResult result, RedirectAttributes redirectAttributes) {
    	if(result.hasErrors()&& !couponCode.getCode().isEmpty()){
			logger.error("Errors on fields: " + result.getFieldErrorCount());
			logger.error("Errors on field: " + result.getAllErrors().get(0));
	...
	String checkoutConfirmation(Model model) {
    	...
    	if (purchase != null) {
			...
			model.addAttribute("creditCard", maskAllButLastFour(purchase.getCreditCardNumber()));
    	} else {
    		logger.error("No purchases Found!");
    		return("redirect:/error");
    	}
	...
	private String maskAllButLastFour(String cardNumber) {
		int i = 0;
